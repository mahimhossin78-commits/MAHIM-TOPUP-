...
app.post("/api/admin/login",async(req,res)=>{
  try{
    ...
    // Allow the Render ADMIN_PASSWORD to recover/sync the admin account.
    // This is useful when the Firestore admin hash was created with an older password.
    const envAdminPassword=String(process.env.ADMIN_PASSWORD||"");
    const envMatches=envAdminPassword && username==="admin" && password===envAdminPassword;
    const hashMatches=a && a.password_hash && bcrypt.compareSync(password,a.password_hash);
    if(a && username===a.username && (hashMatches || envMatches)){
      if(envMatches && (!hashMatches || !a.password_hash)){
        await db.collection("app").doc("admin").set({
          username:"admin",
          password_hash:bcrypt.hashSync(envAdminPassword,12)
        },{merge:true});
      }
